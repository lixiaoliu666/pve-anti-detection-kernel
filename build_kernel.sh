...
sed -i 's/kvm_init_xstate_sizes/printk(KERN_ALERT "kvm.ko AICodo v2.0 Start,ok!!!\\n");\n\tkvm_init_xstate_sizes/g' submodules/ubuntu-kernel/arch/x86/kvm/x86.c # start AICodo flag
...
sed -i 's/\/\* INTR_WINDOW_EXITING/exec_control |= CPU_BASED_RDTSC_EXITING; \/\/Ensure handle_rdtsc() is used.added line AICodo \n\t\/\* INTR_WINDOW_EXITING/g' submodules/ubuntu-kernel/arch/x86/kvm/vmx/vmx.c
...
sed -i 's/__unused_size_checks/printk(KERN_ALERT "kvm-amd.ko AICodo  v2.0 Start,ok!!!\\n");\/\/added line AICodo \n\t__unused_size_checks/g' submodules/ubuntu-kernel/arch/x86/kvm/svm/svm.c
